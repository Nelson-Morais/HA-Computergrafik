...
//
//Erweitert mit GeometryShader für Particle system
// Quellen: https://learnopengl.com/Advanced-OpenGL/Geometry-Shader
...
bool BaseShader::load( const char* VertexShaderFile, const char* FragmentShaderFile, const char* GeometryShaderFile)
{
    unsigned int VSFileSize = 0;
    unsigned int FSFileSize = 0;
    unsigned int GSFileSize = 0;
    char* VSFileData = NULL;
    char* FSFileData = NULL;
    char* GSFileData = NULL;
    ...
    //nur falls load mit geometry aufgerufen wird, sonst shaderprogramm erzeugt die üblichen VS und FS nur. 
    if (GeometryShaderFile != nullptr) {
        GSFileData = loadFile(GeometryShaderFile, GSFileSize);
        if (!GSFileData) {
            std::cout << "Unable to load shader file " << GeometryShaderFile << std::endl;
            return false;
        }
        ShaderProgram = createShaderProgram(VSFileData, FSFileData, GSFileData);
        delete[] GSFileData;
    }
    else {
...
GLuint BaseShader::createShaderProgram( const char* VScode, const char* FScode, const char* GScode)
{
    ModelTransform.identity();
    ...
    //analog zur FS und VS code. Hier nur falls GS existiert.
    if (GScode != nullptr) {
        glCompileShader(GS);
        glGetShaderiv(GS, GL_COMPILE_STATUS, &Success);
        if (Success == GL_FALSE) {
            sprintf(&ShaderLog[WrittenToLog], "GS:");
            WrittenToLog += 3;
            GLsizei Written = 0;
            glGetShaderInfoLog(GS, LogSize - WrittenToLog, &Written, &ShaderLog[WrittenToLog]);
            WrittenToLog += Written;
        }
    }
