...
{
    firstColorLoc = glGetUniformLocation(ShaderProgram, "firstColor");
...
void PhongShader::activate(const BaseCamera& Cam) const
{
    ...
    if (UpdateState & FIRST_TEX_CHANGED)
        glUniform3f(firstColorLoc, firstColor.R, firstColor.G, firstColor.B);
...
void PhongShader::firstColorSet(const Color& c) {
    this->firstColor = c;
    UpdateState |= FIRST_TEX_CHANGED;
}
